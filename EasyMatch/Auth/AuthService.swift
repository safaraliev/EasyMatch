...
//  EasyMatch
//
//  Created by Нуридин Сафаралиев on 12/11/24.
//
...
class AuthService {
    ...
    public func uploadInfo(userAge: String, completion: @escaping (Error?) -> Void) {

        guard let userID = Auth.auth().currentUser?.uid else {
            completion(NSError(domain: "", code: 404, userInfo: [NSLocalizedDescriptionKey: "User not logged in"]))
            return
        ...
        db.collection("users")
            .document(userID)
            .setData(["age" : userAge], merge: true) { error in
                completion(error)
            }
        
        db.collection("users")
            .document(userID)
            .setData(["age": userAge], merge: true)
    }
}
